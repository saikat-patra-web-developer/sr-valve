      ...
      <section className="bg-slate-50 border-y border-slate-200 py-12 sm:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn direction="up" className="text-center max-w-2xl mx-auto mb-9">
            <span className="text-[11px] font-bold text-slate-400 tracking-[0.24em] uppercase">Project experience</span>
            <h2 className="mt-2 text-2xl sm:text-3xl font-extrabold text-[#0d2857]">Supporting essential infrastructure sectors</h2>
          </FadeIn>
          <StaggerContainer className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {sectors.map(({ title, path, image, icon: Icon, description, highlights }) => (
              <StaggerItem
                key={path}
                hoverEffect={true}
                className="group bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-xl transition-all"
              >
                <Link to={path} className="block h-56 overflow-hidden bg-slate-100">
                  <img src={image} alt={title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                </Link>
                <div className="p-6">
                  <div className="w-11 h-11 rounded-lg bg-[#0d2857] flex items-center justify-center -mt-11 relative mb-5 shadow-lg border-4 border-white box-content">
                    <Icon className="w-5 h-5 text-white" />
                  </div>
                  <h3 className="text-xl font-extrabold text-[#0d2857]">{title}</h3>
                  <p className="mt-3 text-sm text-slate-600 leading-6">{description}</p>
                  ...
                  <Link to={path} className="mt-6 inline-flex items-center gap-2 font-bold text-sm text-[#0d2857] group-hover:text-[#f37021] transition-colors">
                    Explore sector experience <ArrowRight className="w-4 h-4" />
                  </Link>
                </div>
              </StaggerItem>
            ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn direction="up">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div>
                <h2 className="text-2xl font-extrabold">Let our experience support your next project</h2>
                <p className="mt-2 text-sm text-slate-300">Discuss project conditions, valve selection and technical requirements with our engineering team.</p>
              </div>
